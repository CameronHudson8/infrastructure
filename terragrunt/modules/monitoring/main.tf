...
data "kubernetes_namespace" "api_server" {
  metadata {
    name = "kube-system"
  }
}

resource "kubernetes_manifest" "api_server_pod_monitor" {
  manifest = {
    apiVersion = "monitoring.coreos.com/v1"
    kind       = "PodMonitor"
    metadata = {
      name      = "api-server"
      namespace = data.kubernetes_namespace.api_server.metadata[0].name
    }
    spec = {
      jobLabel = "component"
      namespaceSelector = {
...
resource "kubernetes_network_policy" "allow_api_server_ingress_from_prometheus" {
  metadata {
    name      = "allow-api-server-ingress-from-prometheus"
    namespace = data.kubernetes_namespace.api_server.metadata[0].name
  }
  spec {
    ingress {
...
resource "kubernetes_network_policy" "allow_prometheus_egress_to_api_server" {
  metadata {
    name      = "allow-prometheus-egress-to-api-server"
    namespace = kubernetes_namespace.monitoring.metadata[0].name
  }
  spec {
    egress {
      to {
        namespace_selector {
          match_labels = {
            "kubernetes.io/metadata.name" = data.kubernetes_namespace.api_server.metadata[0].name
          }
        }
        pod_selector {
          match_labels = {
